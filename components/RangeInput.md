...
A range input is a UI/UX component that provides a slider control for selecting a numeric value within a defined range. It uses the native HTML `<input type="range">` element, which renders as a draggable track and handle. The component supports configurable minimum, maximum, and step values, and provides a controlled value prop for two-way data binding. An aria-label ensures the control is accessible to screen readers and assistive technologies.
...
- `value`: number (default: 50) -- current numeric value, controlled with `value + onChange`
...
```tsx
<RangeInput label="Volume" value + onChange min={0} max={100} />
```

```tsx
<RangeInput label="Price range" value={price} onChange={setPrice} min={10} max={500} step={10} />
```
