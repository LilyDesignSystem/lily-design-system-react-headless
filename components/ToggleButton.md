...
A toggle is a UI/UX component that provides a binary on/off control using a button with `role="switch"`. It communicates its state through `aria-checked` and supports both click and keyboard (Space key) interaction. The pressed state is controlled for two-way data flow.
...
- Supports controlled component usage for the `pressed` prop
...
- `pressed`: boolean (default: false) -- whether the toggle is on; controlled
...
```tsx
<ToggleButton label="Dark mode" pressed={isPressed} onChange={setIsPressed} />
```
