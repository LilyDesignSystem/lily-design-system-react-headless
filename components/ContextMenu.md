...
- Renders a `<div>` with `role="menu"` and `aria-label`, conditionally shown when `open` is true
- Supports controlled component usage for the `open` prop
- `useRef` tracks the menu element reference for focus management
- `useEffect` focuses the first `[role="menuitem"]` when the menu opens
...
- `open`: boolean (default: false) -- whether the menu is visible; controlled
- `children`: ReactNode (required) -- menu item content (should include `role="menuitem"` elements)

## Usage

```tsx
<ContextMenu label="Actions" open={isOpen} onChange={setIsOpen}>
