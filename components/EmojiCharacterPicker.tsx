...
//   children — ReactNode, required. The grid content containing emoji characters.
...
export interface EmojiCharacterPickerProps {
    className?: string;
    [key: string]: unknown;
}

export default function EmojiCharacterPicker({
    className = "",
    label,
    children,
    ...restProps
}: EmojiCharacterPickerProps) {
    return (
        <div
        className={`emoji-character-picker ${className}`}
        role="grid"
        aria-label={label}
        {...restProps}
        >
