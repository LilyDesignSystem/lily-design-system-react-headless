...
- Navigation content is conditionally rendered, removing it from the DOM when closed
...
- `open`: boolean (default: false) -- controlled boolean indicating whether the navigation panel is visible
...
```tsx
<HamburgerMenu label="Main menu" open={isOpen} onChange={setIsOpen}>
  <nav>
    <ul>
      <li><a href="/">Home</a></li>
