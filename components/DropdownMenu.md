...
- Uses `useRef` to track the `menuRef` DOM reference for querying menu items
- Uses `useEffect` to auto-focus the first `[role="menuitem"]` element when the menu opens
...
```tsx
<DropdownMenu label="Options" open={isOpen} onChange={setIsOpen}>
