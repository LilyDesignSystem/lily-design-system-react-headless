...
- No internal state management beyond the controlled value -- relies on native input behavior

## Props

- `label`: string (required) -- accessible name applied via `aria-label`
- `value`: number (default: 50) -- current slider value; controlled
...
```tsx
<Slider label="Volume" value + onChange min={0} max={100} step={5} />
```
