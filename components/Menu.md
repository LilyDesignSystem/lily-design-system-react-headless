...
- Uses `useRef` to track the `menuRef` element reference for querying child items.
...
- `children`: ReactNode (required) -- menu item elements to render inside the menu (should have `role="menuitem"` and `tabindex="-1"`).
...
<Menu label="File actions">
    {actions.map(action => (
        <div role="menuitem" tabindex="-1" onClick={() => handleAction(action)} key={action.label}>
            {action.label}
        </div>
    ))}
</Menu>
