...
- `children`: ReactNode (required) -- slide content to render inside the carousel container.
...
<Carousel label="Featured products">
    {slides.map((slide, i) => (
        <div role="group" aria-roledescription="slide" aria-label={`Slide ${i + 1} of ${slides.length}`} key={i}>
            {slide.content}
        </div>
    ))}
</Carousel>
