...
- Uses `useRef` to track the `listRef` element reference for querying child options.
...
- `children`: ReactNode (required) -- option elements to render inside the listbox (should have `role="option"` and `tabindex="-1"`).
