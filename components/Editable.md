...
- Supports controlled component usage for both `value` and `editing` props
- Uses `useState()` for the internal draft value during editing
...
- `value`: string (default: "") -- current text value; controlled
- `label`: string (required) -- accessible name for the editable control, applied via `aria-label`
- `editing`: boolean (default: false) -- whether the component is in edit mode; controlled
- `disabled`: boolean (default: false) -- whether editing is disabled

## Usage

```tsx
<Editable label="Name" value + onChange />
```

```tsx
<Editable label="Title" value={title} onChange={setTitle} editing={isEditing} onEditingChange={setIsEditing} />
```
