...
```tsx
<FiveFaceRatingPickerButton value={1} label="Very bad" selected={rating === 1} onClick={() => setRating(1)} />
<FiveFaceRatingPickerButton value={2} label="Bad" selected={rating === 2} onClick={() => setRating(2)} />
<FiveFaceRatingPickerButton value={3} label="Okay" selected={rating === 3} onClick={() => setRating(3)} />
<FiveFaceRatingPickerButton value={4} label="Good" selected={rating === 4} onClick={() => setRating(4)} />
<FiveFaceRatingPickerButton value={5} label="Very good" selected={rating === 5} onClick={() => setRating(5)} />
```
