...
- Supports controlled component usage for the `checked` prop
- Spreads `restProps` onto the input element for consumer extensibility

## Props

- `checked`: boolean (default: false) -- controlled boolean indicating whether the checkbox is checked
...
```tsx
<CheckboxInput label="Accept terms" checked={isChecked} onChange={setIsChecked} />
```
...
```tsx
<CheckboxInput label="Required field" checked={isChecked} onChange={setIsChecked} required name="agree" value="yes" />
```
