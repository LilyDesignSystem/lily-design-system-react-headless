...
Unlike a single-line text input, a text-area-input provides a larger, scrollable text area that supports multiple lines. The component uses `aria-label` for accessible naming and supports controlled component usage for the value.
...
- `value`: string (default: "") -- controlled text content of the text-area-input
...
```tsx
<TextAreaInput label="Comments" value + onChange rows={5} />
```
...
```tsx
<TextAreaInput label="Notes" value + onChange disabled={isReadOnly} />
```
