...
A React 19 component that renders an inline `<span>` for displaying United Kingdom's National Insurance Number (NINO) read-only.

## When to use it

- Displaying a United Kingdom National Insurance Number.

## When not to use it

- Use `UnitedKingdomNationalInsuranceNumberInput` for the editable input counterpart.
