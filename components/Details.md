...
The native `<details>` element provides built-in accessibility support without requiring custom ARIA attributes. Screen readers automatically announce the expanded or collapsed state. The component exposes a controlled `open` prop so parent components can programmatically control or react to the disclosure state.
...
- Accepts a `children` ReactNode for the expandable content area
...
- `open`: boolean (default: false) -- whether the details section is expanded; controlled
