...
```tsx
<RedAmberGreenPickerButton value="red" label="Red" selected={status === "red"} onClick={() => setStatus("red")} />
<RedAmberGreenPickerButton value="amber" label="Amber" selected={status === "amber"} onClick={() => setStatus("amber")} />
<RedAmberGreenPickerButton value="green" label="Green" selected={status === "green"} onClick={() => setStatus("green")} />
```
