...
- Supports controlled component usage for both `x` and `y` coordinates (range 0-100)
...
- `x`: number (default: `0`) -- horizontal position (0-100, saturation); controlled
- `y`: number (default: `0`) -- vertical position (0-100, brightness); controlled
...
```tsx
<ColorPicker label="Color saturation and brightness" x={xPos} onXChange={setXPos} y={yPos} onYChange={setYPos} />
```
