...
for (const file of testFiles) {
  ...
  result = result.replace(
    ...
    (match, propsStr) => {
      ...
      // textSnippet("text")
      const textMatch = props.match(
        /children:\s*textSnippet\(["']([^"']*)["']\)/,
      );
      if (textMatch) {
        childContent = textMatch[1];
        props = props.replace(
          /,?\s*children:\s*textSnippet\(["'][^"']*["']\)/,
          "",
        );
      }
      ...
      if (treeMatch) {
        childContent =
          '<li role="treeitem" tabIndex={-1}>Item 1</li><li role="treeitem" tabIndex={-1}>Item 2</li>';
