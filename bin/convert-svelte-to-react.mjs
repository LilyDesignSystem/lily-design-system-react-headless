 ...
 *
 * Reads from ../lily-plus-svelte/components/
...
const SVELTE_DIR = join(
  import.meta.dirname,
  "../../lily-plus-svelte/components",
);
...
function extractProps(scriptContent) {
  ...
  if (!propsMatch)
    return { entries: [], typeBlock: "", hasChildren: false, hasBindable: [] };
...
function parseDestructuring(str) {
  ...
  for (const line of lines) {
    ...
    if (bindableMatch) {
      entries.push({
        name: bindableMatch[1],
        defaultValue: bindableMatch[2] || undefined,
        isBindable: true,
      });
      continue;
    }
...
function extractLogic(scriptContent, hasBindable) {
  ...
  while ((funcMatch = funcRegex.exec(scriptContent)) !== null) {
      ...
      (match, refName) => {
        if (
          scriptContent.includes(
            `${refName}: HTMLElement | undefined = $state(undefined)`,
          ) ||
    ...
    funcBody = funcBody.replace(
      /(\w+Ref)\.querySelectorAll/g,
      "$1.current.querySelectorAll",
    );
    funcBody = funcBody.replace(
      /(\w+Ref)\.querySelector\b/g,
      "$1.current.querySelector",
    );
...
function convertIfBlocks(jsx) {
  ...
  while (maxIterations-- > 0) {
    ...
    // {#if cond}...{/if}
    const ifMatch = result.match(/\{#if\s+([^}]+)\}([\s\S]*?)\{\/if\}/);
    if (ifMatch) {
...
function buildReactComponent(componentName, svelteContent) {
  ...
  const { entries, typeBlock, hasChildren, hasBindable } =
  ...
  const hasRef =
    scriptContent.includes("$state(undefined)") &&
    template.includes("bind:this");
  const hasState =
    scriptContent.includes("$state(") &&
    !template.includes("bind:this") &&
    !(
      entries.every((e) => !e.isBindable) &&
      scriptContent.match(/\$state\(undefined\)/)
    );
  ...
  const propsInterface = buildPropsInterface(
    componentName,
    typeBlock,
    hasChildren,
    hasBindable,
  ...
  let refDecl = "";
  if (hasRef) {
    const refMatch = scriptContent.match(
      /let\s+(\w+)(?::\s*[^=]+)?\s*=\s*\$state\(undefined\)/,
    );
    if (refMatch) {
...
function convertRenderCall(propsStr) {
  ...
  const childrenMatch = props.match(
    /children:\s*textSnippet\(["']([^"']*)["']\)/,
  );
  ...
  try {
    ...
    try {
      testContent = readFileSync(
        join(SVELTE_DIR, `${componentName}.test.ts`),
        "utf-8",
      );
    } catch {}
    ...
    let mdContent = null;
    try {
      mdContent = readFileSync(
        join(SVELTE_DIR, `${componentName}.md`),
        "utf-8",
      );
    } catch {}
